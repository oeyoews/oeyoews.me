...
layout.js 里面是可以使用use client, 不会导致子组件全部变成 client

## RSC

注意: RSC 不是ssr 的升级(他们是两种不同的概念)
...
React renders Server Components into a special data format(可序列的数据化的数据, 用于server=> 向 client的数据传递) called the React Server Component Payload (RSC Payload), which includes references to Client Components(占位符).
(rsc payload 和client component进行hydration(水合), 渲染html, 使其具备交互能力)
The React Server Components Payload is used to reconcile the Client and Server Component trees, and update the DOM.(维护dom tree)
...
(之所以不能嵌入就是因为最小边界时文件不是组件), props 可以保证完全区分组件类型, 使用占位符代替, 注意这里的数据必须时可序列化的, 对象/函数是不能作为props 传递的

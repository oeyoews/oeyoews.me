...
nextjs 中的usesearchparams 不是完整的, useSearchParams is a Client Component hook that lets you read the current URL's query string.
...
> 由于nextjs 的路由缓存, revalidate 即使成功, 有时也会看到过去的数据, revalidate 也不是及时会更新, 需要等到新的数据成功获取, 类似于service worker 或者swr

import ToastDemo from '@/components/ToasterWrapper';
...
> NextJs 13 让 RSC 有重新出现在人们眼前, React 已经一年没有发版, React 实在进行底层库的转变(越来越像一个库)???

## Nextjs Mdx

由于(@next/mdx 不支持 frontmatter) 扩展了 metadata, 想要在页面除了展示 pagd.mdx 的内容, 还需要自定义展示 metadata 的内容, 达到 frontmetadata 的作用, 使用二级动态 layout(这不是很符合 nextjs 直觉), 如果在根目录使用 loading, 二级 layout 的内容不会即使刷新(page.mdx 的内容是正常工作的), 需要二次刷新
...
Tailwind 在 NextJs 里面似乎很受支持, 直接在 CNA 里面内置了选项支持
